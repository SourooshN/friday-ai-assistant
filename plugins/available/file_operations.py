...
class FileOperationsPlugin:
        ...
        return {
            ...
            "update_file": {
                ...
                "parameters": {
                    ...
                    "mode": {
                        "type": "string",
                        "description": "Update mode: overwrite, append, prepend",
                        "default": "overwrite"
                    },
                    "encoding": {
                        "type": "string",
            ...
            "delete_directory": {
                ...
                "parameters": {
                    ...
                    "recursive": {
                        "type": "boolean",
                        "description": "Delete recursively (non-empty directories)",
                        "default": False
                    },
                    "confirm": {
                        "type": "boolean",
            ...
            "search_files": {
                ...
                "parameters": {
                    ...
                    "file_type": {
                        "type": "string",
                        "description": "File type filter (extension)",
                        "default": None
                    },
                    "max_results": {
                        "type": "integer",
    ...
    def _update_file(self, file_path: str, content: str, mode: str = "overwrite", encoding: str = "utf-8") -> Dict[str, Any]:
        ...
        try:
            ...
            if mode == "overwrite":
                ...
                return {
                    "success": False,
                    "error": f"Invalid update mode: {mode}"
                }
    ...
    def _delete_file(self, file_path: str, confirm: bool = False) -> Dict[str, Any]:
        ...
        try:
            if not confirm:
                return {
                    "success": False,
                    "error": "File deletion requires confirmation (set confirm=True)"
                }
    ...
    # Folder Operations
    def _list_directory(self, directory_path: str = ".", show_hidden: bool = False,
    ...
    def _delete_directory(self, directory_path: str, recursive: bool = False, confirm: bool = False) -> Dict[str, Any]:
        ...
        try:
            if not confirm:
                return {
                    "success": False,
                    "error": "Directory deletion requires confirmation (set confirm=True)"
                }
            ...
            if not recursive and any(path.iterdir()):
                return {
                    "success": False,
                    "error": f"Directory is not empty: {directory_path} (use recursive=True to force)"
                }
    ...
    def _copy_file(self, source_path: str, destination_path: str, overwrite: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if dst_path.exists() and not overwrite:
                return {
                    "success": False,
                    "error": f"Destination exists: {destination_path} (use overwrite=True to replace)"
                }
            ...
            if src_path.is_file():
                ...
                return {
                    "success": False,
                    "error": f"Unsupported file type: {source_path}"
                }

            dst_size = dst_path.stat().st_size if dst_path.is_file() else sum(
    ...
    def _search_files(self, base_path: str = ".", pattern: str = "*",
                     content_search: Optional[str] = None, file_type: Optional[str] = None,
        ...
        try:
            ...
            if not search_path.exists():
                return {
                    "success": False,
                    "error": f"Search path not found: {base_path}"
                }

            if not search_path.is_dir():
                return {
                    "success": False,
                    "error": f"Search path is not a directory: {base_path}"
                }
